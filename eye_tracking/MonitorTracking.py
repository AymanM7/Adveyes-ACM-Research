...
screen_position_file = "/Users/ani/workspaces/github.com/anishalle/adveyes/eye_tracking/screen_position.txt"
# ===== NEW: NDJSON output for gaze records =====
import pathlib

# Per-session output: create folder and unique filename (timestamped)
out_dir = pathlib.Path(r"/Users/ani/workspaces/github.com/anishalle/adveyes/eye_tracking/gaze_sessions")
...
def vector_triplet_from_matrix(R):
    """Return right, up, forward vectors from rotation matrix R as lists."""
    right = R[:, 0].tolist()
    up = (-R[:, 1]).tolist()
    forward = (-R[:, 2]).tolist()
...
def compute_gaze_quaternion(gaze_dir, up_hint=None):
    """Compute a quaternion [x,y,z,w] for a gaze direction vector.

    gaze_dir: iterable of 3 floats (points toward gaze target in world/head space)
    up_hint: optional up vector (if None, world up [0, -1, 0] is used)
    """
    g = np.asarray(gaze_dir, dtype=float)
    if np.linalg.norm(g) < 1e-9:
        raise ValueError("gaze_dir too small")
    f = g / np.linalg.norm(g)
    ...
    if np.linalg.norm(right) < 1e-6:
        # fallback to global X
        right = np.array([1.0, 0.0, 0.0], dtype=float)
    right = right / np.linalg.norm(right)
    up = np.cross(f, right)
    up = up / np.linalg.norm(up)

    R_gaze = np.stack([right, up, f], axis=1)
    return quaternion_from_rotation_matrix(R_gaze)


def append_gaze_record(timestamp_ms, head_R=None, gaze_quat=None, gaze_dir=None, per_eyes=None, gazeBehaviorType=None, duration=None):
    """Append a JSON line to gaze_output_file. Uses quaternion fields when available, otherwise vectors."""
    rec = {
        "timeMs": int(timestamp_ms),
    }

    # Head rotation
    if head_R is not None:
        try:
            q = quaternion_from_rotation_matrix(head_R)
            rec.update({
                "headRotation": {"x": q[0], "y": q[1], "z": q[2], "w": q[3]}
            })
        except Exception:
            r, u, f = vector_triplet_from_matrix(head_R)
            rec.update({
                "headUp": u, "headRight": r, "headForward": f
            })

    # Gaze quaternion or direction
    if gaze_quat is not None:
        rec.update({
            "eyeGazeRotation": {"x": gaze_quat[0], "y": gaze_quat[1], "z": gaze_quat[2], "w": gaze_quat[3]}
        })
    elif gaze_dir is not None:
        rec.update({"eyeGazeDirection": [float(v) for v in gaze_dir]})

    if gazeBehaviorType is not None:
        rec["gazeBehaviorType"] = gazeBehaviorType
    if duration is not None:
        rec["duration"] = duration

    if per_eyes is not None:
        rec["perEyes"] = per_eyes

    # Write as NDJSON (one JSON object per line)
    try:
        with open(gaze_output_file, 'a', encoding='utf-8') as f:
            f.write(json.dumps(rec) + "\n")
    except Exception as e:
        print(f"[GazeWriteError] {e}")
...
def create_monitor_plane(head_center, R_final, face_landmarks, w, h, 
    ...
    mon_w_cm, mon_h_cm = 35.678, 25.19
...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        ...
        # --- NEW: build and append gaze record per frame ---
        try:
            ts_ms = int(time.time() * 1000)

            # Head rotation: prefer R_final (3x3) if available
            head_R = R_final if 'R_final' in locals() else None

            # Combined gaze quaternion: if avg_combined_direction available, compute quaternion; fall back to direction
            gaze_quat = None
            gaze_dir = None
            if 'avg_combined_direction' in locals() and avg_combined_direction is not None:
                gaze_dir = _normalize(np.asarray(avg_combined_direction, dtype=float))
                try:
                    gaze_quat = compute_gaze_quaternion(gaze_dir)
                except Exception:
                    gaze_quat = None

            # Per-eye info: pull iris/nose/pupil if available
            per_eyes = []
            try:
                if left_iris is not None:
                    per_eyes.append({
                        "eye": "Left",
                        "eyeInSkullPosition": {"x": float(left_iris.x * w), "y": float(left_iris.y * h)},
                        "eyeOpenness": float(face_landmarks[159].y) if len(face_landmarks) > 159 else None,
                        "pupilDiameter": None
                    })
            except Exception:
                pass
            try:
                if right_iris is not None:
                    per_eyes.append({
                        "eye": "Right",
                        "eyeInSkullPosition": {"x": float(right_iris.x * w), "y": float(right_iris.y * h)},
                        "eyeOpenness": float(face_landmarks[386].y) if len(face_landmarks) > 386 else None,
                        "pupilDiameter": None
                    })
            except Exception:
                pass

            append_gaze_record(ts_ms, head_R=head_R, gaze_quat=gaze_quat, gaze_dir=gaze_dir, per_eyes=per_eyes)
        except Exception as e:
            print(f"[GazeRecordError] {e}")
