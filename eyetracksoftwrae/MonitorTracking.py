...
screen_position_file = "/Users/ani/workspaces/github.com/anishalle/adveyes/eyetracksoftwrae/outputfile.txt"
...
def render_debug_view_orbit(
    ...
    # Ensure debug buffer is contiguous and safely call imshow
    try:
    ...
    try:
        ...
        try:
            print(f"[imshow ERROR debug] {e!r}; debug shape={getattr(debug,'shape',None)}, dtype={getattr(debug,'dtype',None)}, contiguous={debug.flags['C_CONTIGUOUS'] if hasattr(debug,'flags') else 'NA'}")
        except Exception:
            print(f"[imshow ERROR debug] {e!r}; failed to inspect debug buffer")
...
while cap.isOpened():
    ret, frame = cap.read()
    # Defensive: ensure we have a valid frame. On some systems OpenCV may
    # return empty frames intermittently; passing those into macOS image APIs
    # (via cv2.imshow) can trigger CFData range assertions.
    if not ret or frame is None or getattr(frame, 'size', 0) == 0:
        # Wait briefly and continue rather than passing invalid data to imshow
        time.sleep(0.01)
        continue

    # If capture-reported width/height were 0 (sometimes happens), re-evaluate
    # them from the first valid frame we received.
    if w == 0 or h == 0:
        h, w = frame.shape[:2]

    # Ensure frame is uint8 and C-contiguous. Non-contiguous or unexpected
    # dtypes can cause downstream native APIs to miscompute buffer sizes.
    ...
    # Ensure images are contiguous before calling imshow (prevents CFData errors on macOS)
    try:
        debug = np.ascontiguousarray(debug)
        frame = np.ascontiguousarray(frame)
    except Exception:
    ...
    # Defensive imshow with diagnostics: if an exception occurs in the GUI backend
    # (often surfaced as an assertion in CFData on macOS), catch and log details
    try:
        cv2.imshow("Integrated Eye Tracking", frame)
    except Exception as e:
        # Log frame diagnostics to help track down buffer issues
        try:
            print(f"[imshow ERROR] {e!r}; frame shape={getattr(frame,'shape',None)}, dtype={getattr(frame,'dtype',None)}, contiguous={frame.flags['C_CONTIGUOUS'] if hasattr(frame,'flags') else 'NA'}")
        except Exception:
            print(f"[imshow ERROR] {e!r}; failed to inspect frame")
        # Avoid crashing; continue the loop after a short pause
        time.sleep(0.05)
